...
Mproto.mul = function(m) {
  ...
  for(i = 1; i <= 3; ++i) {
    for(j = 1; j <= 3; ++j) {
      if(mmatrix){a.push(this[i][1] * m[1][j] +
      this[i][2] * m[2][j] +
      this[i][3] * m[3][j]);} else {a.push(this[i][j] * m);}
    }
}
  return new Matrix(a);
};
...
Tproto.Measure = function(c,t) {
  ...
  if(t.txtOpt) {
    ...
    if(this.image){tcv.SetImage(this.image, this.iw, this.ih, t.imagePosition, t.imagePadding,
    ...
    if(t.outlineMethod == 'colour') {
      ...
      tcv = new TextCanvas(this.text, f, cw + s, (s * th) + s, cw,
        this.line_widths, t.textAlign, t.textVAlign, s);
      if(this.image){tcv.SetImage(this.image, this.iw + t.outlineIncrease,
...
function TagCanvas(cid,lctr,opt) {
  ...
  if(lctr && this.hideTags) {
    (function(t) {
    if(TagCanvas.loaded){t.HideTags();} else {AddHandler('load', function() { t.HideTags(); }, window);}
    })(this);
  }
...
TCproto.AnimatePosition = function(w, h, t) {
  ...
  stopped = (tc.pitch == 0 && tc.yaw == 0) ? true : false;
...
TCproto.Clicked = function(e) {
  ...
  try {
    if(a && a.tag) {
        if(this.clickToFront === false || this.clickToFront === null){a.tag.Clicked(e);} else {this.TagToFront(a.tag, this.clickToFront, function() {
          a.tag.Clicked(e);
        }, true);}
    }
  } catch(ex) {
...
AddHandler('load',function(){TagCanvas.loaded=1;stopped = false;$('.blind').fadeOut();},window);
})(jQuery);
...
function company_select(s) {
    setTimeout(function(){
        window.top.location.href = $(s).attr('href');
    }, 1020);
}

function resize() {
    var w, h;
    w = $("#myCanvas").innerWidth();
    h = (w > 650) ? 650 : w;
    $("#myCanvas").attr("width", w);
    $("#myCanvas").attr("height", h);
    $("#myCanvas").css("height", h + "px");
    $(".blind").css("line-height", h + "px");
}
...
$(function() {
    if (jQuery.browser.mobile === true) {
        $('.container, .ie_container').css('border', '1px solid #DDDDDD');
    }
    if (! isCanvasSupported()) {
        ...
        if( ! $('#myCanvas').tagcanvas({
            ...
            pinchZoom : true,
            tooltip : "native",
            zoom : (jQuery.browser.mobile === true && window.innerWidth <= 420) ? 0.7 : 1.0,
            zoomMax: 1.15,
            zoomMin: 0.65,
            wheelZoom : (jQuery.browser.mobile === true) ? true : false,
            outlineThickness : 2,
            maxSpeed : 0.025,
            depth : 0.5,
            imageMode : "image",
        ...
        setInterval(function(){
            if (stopped === true && restarting === false) {
                //console.log("stopped detected. restarting in 10sec");
                setTimeout(function(){
                    $('#myCanvas').tagcanvas("setspeed", [0.150, 0.150]);
                    restarting = false;
